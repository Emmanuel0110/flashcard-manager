...
const DotOptions: React.FC<DotOptionsProps> = ({obj, options}) => {
  ...
  return <div className="dotsOptions">
    <div style={{position: 'relative'}}>
      <div className='dots' onClick={onClickDots}></div>
      {isVisible && <div ref={dropdownMenuRef} className="dotsOptionMenu">
        {options.map(({callback, label}, index) => <div key={index} className="optionLabel" onClick={e => onClick(callback)}>{label}</div>)}
      </div>}
    </div>
  </div>
}
