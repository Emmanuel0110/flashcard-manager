...
export default function FlashcardComponent() {
  ...
  useEffect(() => {setAnswerVisible(filter === "Draft" || filter === "To be validated")}, [flashcardId])
  ...
  const reviewIn = (n: number, period: "day" | "week" | "month") => {
    ...
    if (flashcard) {
      editUserFlashcardInfo({ _id: flashcard._id, nextReviewDate: date});
    }
  }
    ...
    <div id="flashCardComponent">
      <div className="buttonHeader">
        ...
        {flashcard?.status === "Published" && (
          <Button onClick={publish}>{flashcard.nextReviewDate instanceof Date ? "Unsubscribe" : "Subscribe"}</Button>
        )}
      </div>
      <div id="flashcard">
        <div id="middle">
          ...
          {answerVisible ? (
            <>
              ...
              {filter == "To be reviewed" && <div id="answerButtons">
                <Button onClick={() => reviewIn(1, "day")} style={{ backgroundColor: "lightblue" }}>
                  I forgot <br /> +1day
                </Button>
                <Button onClick={() => reviewIn(1, "week")}>
                  Not sure <br /> +1week
                </Button>
                <Button onClick={() => reviewIn(1, "month")} style={{ backgroundColor: "blue" }}>
                  I know <br /> +1month
                </Button>
              </div>}
            </>
